...
for f in [f1, f2]:
    for point in (point1, point2):
        for deviations in [deviations1, deviations2]:
            print(('%f+/-%f   ' % (f(point), f.uncertainty(point, deviations))), end=' ') # comma will work in other way in Python3
    print('\n')
# Vectorized example
point = {a:[1, 2, 3], b:0.1}
...
f = dot(M, a) + b
for deviations in [deviations1, deviations2]:
    print(('%s +/- %s     ' % (f(point), f.uncertainty(point, deviations))), end=' ') # comma will work in other way in Python3
# Output: 
