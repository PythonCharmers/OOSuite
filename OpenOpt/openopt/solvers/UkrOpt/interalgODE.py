...
logical_not, argsort, logical_and, vstack, any
...
def interalg_ODE_routine(p, solver):
    ...
    for itn in range(p.maxIter+1):
        ...
        if ind.size != 0: 
            r33 -= ((r34[ind] - r35[ind]) * (r29[ind] - r28[ind])).sum()
            r37 -= (r29[ind] - r28[ind]).sum()
        ind = where(logical_not(r36))[0]
        if ind.size == 0:
            p.istop = 1000
            p.msg = 'problem solved according to required tolerance'
            break
